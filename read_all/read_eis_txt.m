...
function [dataArray, slewRate] = read_eis_txt(filePaths, configFile)
...
function data = readDataFromFile(filepath)
    ...
    if ~headerFound
        % Wenn kein Header gefunden wurde, Datei schließen und erneut öffnen
        fclose(fid);
        fid = fopen(filepath, 'r');
        ...
        % Daten ab der zweiten Zeile lesen
        data = textscan(fid, '%f %f %*f %f %*f %*f %*f %*f %*f %*f', 'Delimiter', '\t', 'MultipleDelimsAsOne', true);
        ...
        if isempty(data{1})
            ...
            % Zeilenindex hinzufügen
            rowIndex = (2:(length(data{1})+1))';
            % Daten in ein Array umwandeln
            data = [rowIndex, data{1}, data{2}, data{3}];
